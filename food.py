...
english_lst=['watermelon', 'pizza', 'scrambled-eggs']
...
food_diction={'watermelon': 2, 'scrambled-eggs': 1, 'pizza': 3, 'BG': 0}
...
class FoodConfig(Config):
    ...
    NUM_CLASSES = 1 + 3  # background + 12 foods
...
class FoodDataset(utils.Dataset):
    ...
    def load_food(self, dataset_dir, subset):
        ...
        for a in annotations:
            ...
            image_path = os.path.join(dataset_dir,a+".png")
            image = skimage.io.imread(image_path)
            height, width = image.shape[:2]

            self.add_image(
                "food",
                image_id=a+".png",  # use file name as a unique image id
                path=image_path,
